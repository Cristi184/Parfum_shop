...
function App() {
    ...
    */
        ...
        if (filter) {
            ...
            shopItems.forEach(item => {
               const name = item.name.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "")
                if (name.includes(filter) || item.price.includes(filter)) {
                    newArray.push(item)
                }
            })
            setItemsShop(newArray)
        } else setItemsShop(shopItems)
    ...
    return (
        <>
            <Router>
                <Header CartItem={CartItem} sendSearchValue={getSearchValue}/>
                <Switch>
                    <Route path='/' exact>
                        <Pages productItems={productItems} addToCart={addToCart} shopItems={itemsShop}
